...
namespace ReimuPlugins.Th18Replay;
...
[ReimuPlugin]
internal sealed class Plugin : ReimuPluginRev1<Plugin.ColumnKey>
{
    private const string ValidSignature = "t18r";

    private static readonly string[] PluginInfoImpl =
    {
        "REIMU Plug-in For 東方虹龍洞 Ver2.2.0 (C) 2021 IIHOSHI Yoshinori\0",
        "東方虹龍洞\0",
        "th18_*.rpy\0",
        "東方虹龍洞 リプレイファイル (th18_*.rpy)\0",
    };
        ...
        try
        {
            ...
            if (size == 0u)
            {
                ...
                number = ReplayDataBase.GetNumberFromPath(
                    path, @"^th18_(\d{2})\.rpy$", @"^th18_ud(.{0,4})\.rpy$");
            }
