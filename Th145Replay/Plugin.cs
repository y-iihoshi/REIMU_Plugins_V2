...
internal sealed class Plugin : ReimuPluginRev1<Plugin.ColumnKey>
{
    ...
    {
        "REIMU Plug-in For 東方深秘録 Ver2.2.0 (C) 2017 IIHOSHI Yoshinori\0",
        "東方深秘録\0",
        "*.rep\0",
        "東方深秘録 リプレイファイル (*.rep)\0",
    };
...
#pragma warning restore SA1413 // Use trailing comma in multi-line initializers
    ...
    private static IReadOnlyDictionary<ColumnKey, Func<ReplayData, string>> InitializeFileInfoGetters()
    {
        return new Dictionary<ColumnKey, Func<ReplayData, string>>
        {
            ...
            {
                ColumnKey.SpellCard2,
                (data) => (data.GameMode == GameMode.Story) ? "-" : data.SpellCard2Name
            },
            {
                ColumnKey.Profile2,
