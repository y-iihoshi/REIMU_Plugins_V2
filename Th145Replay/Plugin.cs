...
namespace ReimuPlugins.Th145Replay
{
    ...
    public static class Plugin
    {
        ...
        private sealed class PluginImpl : ReimuPluginRev1<PluginImpl.ColumnKey>
        {
            ...
            {
                "REIMU Plug-in For 東方深秘録 Ver2.00 (C) IIHOSHI Yoshinori, 2017\0".ToCP932(),
                "東方深秘録\0".ToCP932(),
                "*.rep\0".ToCP932(),
                "東方深秘録 リプレイファイル (*.rep)\0".ToCP932(),
            };
...
#pragma warning restore SA1413 // Use trailing comma in multi-line initializers
            ...
            private static Dictionary<ColumnKey, Func<ReplayData, string>> InitializeFileInfoGetters()
            {
                return new Dictionary<ColumnKey, Func<ReplayData, string>>
                {
                    ...
                    {
                        ColumnKey.SpellCard1,
                        (data) => data.SpellCard1Name.ToCP932()
                    },
                    {
                        ColumnKey.Profile1,
                    ...
                    {
                        ColumnKey.SpellCard2,
                        (data) => ((data.GameMode == GameMode.Story) ? "-" : data.SpellCard2Name).ToCP932()
                    },
                    {
                        ColumnKey.Profile2,
                        (data) => ((data.GameMode == GameMode.Story)
